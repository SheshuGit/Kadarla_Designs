    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-emerald-600">Manage your product inventory</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <div className="bg-white rounded-xl p-3 shadow-md border border-emerald-100">
          <p className="text-sm text-emerald-600 mb-1">Total Items</p>
        ...
        <div className="bg-white rounded-xl p-4 shadow-md border border-emerald-100">
          <p className="text-sm text-emerald-600 mb-1">Active Items</p>
          <p className="text-2xl font-bold text-emerald-900">{items.filter(i => i.isActive).length}</p>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-md border border-emerald-100">
          <p className="text-sm text-emerald-600 mb-1">Inactive Items</p>
          <p className="text-2xl font-bold text-emerald-900">{items.filter(i => !i.isActive).length}</p>
        </div>
        <div className="bg-white rounded-xl p-4 shadow-md border border-emerald-100">
          <p className="text-sm text-emerald-600 mb-1">Filtered Results</p>
          <p className="text-2xl font-bold text-emerald-900">{filteredItems.length}</p>
        </div>
      </div>
            ...
            >
              ...
              <div className="p-5">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="text-lg font-bold text-emerald-900 line-clamp-2 flex-1">
                    {item.title}
                  </h3>
                </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    {item.discount && item.discount > 0 && isDiscountActive(item) ? (
                      <div>
                        <p className="text-lg text-gray-500 line-through">₹{item.price.toLocaleString('en-IN')}</p>
                      ...
                      <div>
                        <p className="text-2xl font-bold text-emerald-900">₹{item.price.toLocaleString('en-IN')}</p>
                        {item.discount && item.discount > 0 && !isDiscountActive(item) && (
                  ...
                  <span className="px-3 py-1 bg-mint-50 text-emerald-700 rounded-lg text-sm font-medium">
                    {item.category}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2 pt-4 border-t border-emerald-100">
                  ...
                  <button
                    ...
                    className="p-2 bg-mint-50 text-emerald-700 rounded-lg hover:bg-emerald-100 transition-colors"
                    title="View Details"
                  >
                    <Eye size={18} />
                  </button>
